...
const GITHUB_REPO = 'liumubai/movabel';
...
export async function getLatestRelease(): Promise<ReleaseInfo> {
  ...
  try {
    ...
    const releaseInfo: ReleaseInfo = {
      ...
      downloadLinks: {
        macArm:
          downloadLinks.macArm || `${baseUrl}/Movabel_${version.replace('v', '')}_aarch64.dmg`,
        macIntel:
          downloadLinks.macIntel || `${baseUrl}/Movabel_${version.replace('v', '')}_x64.dmg`,
        windows:
          downloadLinks.windows || `${baseUrl}/movabel_${version.replace('v', '')}_x64_en-US.msi`,
        linux: downloadLinks.linux || `${baseUrl}/movabel_x86_64-unknown-linux-gnu.AppImage`,
      },
    };
