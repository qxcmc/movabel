...
import movabelLogo from '@/assets/movabel-logo.png';
...
/**
 * Validate that a health response has the expected Movabel-specific shape.
 * Prevents misidentifying an unrelated service on the same port.
 */
function isMovabelHealthResponse(health: HealthResponse): boolean {
  return (
...
function MainApp() {
  ...
  useEffect(() => {
    if (!platform.metadata.isTauri) {
      ...
      if (currentServerUrl !== serverUrl && isLoopbackMovabelServerUrl(currentServerUrl)) {
    ...
    if (!import.meta.env?.PROD) {
      ...
      window.__movabelServerStartedByApp = false;
      return;
    }
    ...
    platform.lifecycle
      ...
      .then((serverUrl) => {
        ...
        window.__movabelServerStartedByApp = true;
      })
      .catch((error) => {
        ...
        // until it responds with a valid Movabel payload, then transition to
        // the main UI.
        console.log('Falling back to health-check polling...');
        const pollInterval = setInterval(async () => {
          try {
            const health = await apiClient.getHealth();
            if (!isMovabelHealthResponse(health)) {
              console.log('Health response is not from a Movabel server, keep polling...');
              return;
            }
            console.log('External Movabel server detected via health check');
        ...
        setTimeout(() => {
          ...
          serverStartingRef.current = false;
          setStartupError(
            'Could not connect to a Movabel server within 2 minutes. ' +
