...
directly via URL; older stdio-only clients use the ``movabel-mcp`` shim
...
def build_mcp_server() -> FastMCP:
    """Create the FastMCP instance with Movabel tools registered."""
    mcp = FastMCP(
        name="movabel",
        instructions=(
            "Movabel is a local voice I/O layer. Use `movabel.speak` to "
            "play text in a voice profile, `movabel.transcribe` for "
...
def compose_lifespan(*lifespans):
    ...
    Used by ``create_app`` to run the existing Movabel startup/shutdown
