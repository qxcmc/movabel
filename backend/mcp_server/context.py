...
MCP clients identify themselves via an ``X-Movabel-Client-Id`` HTTP header
(direct-HTTP clients set it in their MCP config; the stdio shim forwards it
from the ``MOVABEL_CLIENT_ID`` env var). Middleware copies the value into a
...
# host-filesystem access to loopback callers (see movabel.transcribe).
...
# Endpoints that consume X-Movabel-Client-Id for its MCP-semantic
...
#
# - /mcp — FastMCP tool calls (movabel.speak, movabel.transcribe, …)
...
#   false stamps.
# - /speak — REST mirror of movabel.speak for non-MCP agents (shell
...
class ClientIdMiddleware(BaseHTTPMiddleware):
    """Copy X-Movabel-Client-Id into a ContextVar and stamp last_seen_at
