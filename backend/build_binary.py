...
def build_server(cuda=False):
    ...
    Args:
        cuda: If True, build with CUDA support and name the binary
              movabel-server-cuda instead of movabel-server.
    """
    backend_dir = Path(__file__).parent

    binary_name = "movabel-server-cuda" if cuda else "movabel-server"
    ...
    args.extend(
        [
            ...
            "--hidden-import",
            "loguru",
            # MCP server — Streamable-HTTP endpoint and the 4 movabel.* tools.
...
def build_shim():
    """Build the movabel-mcp stdio shim as a tiny standalone binary.

    This is the bridge for MCP clients that only speak stdio — it proxies
    JSON-RPC to the main movabel-server's /mcp endpoint. Keep it small: no
    ...
    os.chdir(backend_dir)
    PyInstaller.__main__.run(args)
    logger.info("Shim built: %s", backend_dir / "dist" / "movabel-mcp")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build movabel binaries")
    parser.add_argument(
        "--cuda",
        action="store_true",
        help="Build CUDA-enabled binary (movabel-server-cuda)",
    )
    parser.add_argument(
        "--shim",
        action="store_true",
        help="Build the movabel-mcp stdio shim binary instead of the server",
    )
