...
def get_profile_by_name_or_id(name_or_id: str, db: Session):
    """Resolve a profile by its id or name. Returns None if not found."""
    profile = db.query(DBVoiceProfile).filter_by(id=name_or_id).first()
    if profile is not None:
        return profile
    return db.query(DBVoiceProfile).filter_by(name=name_or_id).first()
