"""
Entry point for PyInstaller-bundled movabel server.
...
if "--version" in sys.argv:
    ...
    print(f"movabel-server {__version__}")
    sys.exit(0)
...
logger.info("=" * 60)
logger.info("movabel-server starting up...")
...
if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(description="movabel backend server")
        parser.add_argument(
            "--host",
            type=str,
            default="127.0.0.1",
        ...
        # movabel-server-cuda → sets MOVABEL_BACKEND_VARIANT=cuda
        import os
        binary_name = os.path.basename(sys.executable).lower()
        if "cuda" in binary_name:
            os.environ["MOVABEL_BACKEND_VARIANT"] = "cuda"
            logger.info("Backend variant: CUDA")
        else:
            os.environ["MOVABEL_BACKEND_VARIANT"] = "cpu"
