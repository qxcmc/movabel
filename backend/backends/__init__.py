...
def _get_cloud_tts_configs() -> list[ModelConfig]:
    """Return model configs for cloud-based TTS engines (no local download)."""
    return [
        ModelConfig(
            model_name="openai-tts-1",
            display_name="OpenAI TTS-1",
            engine="openai_tts",
            hf_repo_id="",  # Not applicable — cloud API
            model_size="tts-1",
            size_mb=0,
            supports_instruct=False,
            languages=OPENAI_LANGUAGES if "OPENAI_LANGUAGES" in dir() else
        ...
        ModelConfig(
            ...
            hf_repo_id="",
            model_size="eleven_multilingual_v2",
            size_mb=0,
            languages=["zh", "en", "ja", "ko", "de", "fr", "ru", "pt", "es", "it",
                       "he", "ar", "da", "el", "fi", "hi", "ms", "nl", "no", "pl", "sv", "sw", "tr"],
        ),
        ModelConfig(
            model_name="elevenlabs-turbo-v2-5",
            display_name="ElevenLabs Turbo v2.5",
            engine="elevenlabs_tts",
            hf_repo_id="",
            model_size="eleven_turbo_v2_5",
            size_mb=0,
            languages=["en", "de", "fr", "es", "it", "pt", "pl", "hi", "zh", "ja", "ko"],
        ),
        ModelConfig(
            model_name="elevenlabs-flash-v2-5",
            display_name="ElevenLabs Flash v2.5",
            engine="elevenlabs_tts",
            hf_repo_id="",
            model_size="eleven_flash_v2_5",
            size_mb=0,
            languages=["en", "de", "fr", "es", "it", "pt", "pl", "hi", "zh", "ja", "ko"],
        ),
    ]
...
def get_tts_backend_for_engine(engine: str) -> TTSBackend:
    ...
    with _tts_backends_lock:
        ...
        if engine == "qwen":
            ...
            from .openai_tts_backend import OpenAITTSBackend

            backend = OpenAITTSBackend()
        elif engine == "elevenlabs_tts":
            from .elevenlabs_tts_backend import ElevenLabsTTSBackend
