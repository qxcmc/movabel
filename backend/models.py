...
class GenerationRequest(BaseModel):
    ...
    engine: Optional[str] = Field(default="qwen", pattern="^(qwen|qwen_custom_voice|luxtts|chatterbox|chatterbox_turbo|tada|kokoro|openai_tts|elevenlabs_tts)$")
...
class MCPClientBindingResponse(BaseModel):
    ...
    use when a given client_id calls movabel.speak without args, plus an
    ...
    default_engine: Optional[str] = Field(
        None,
        pattern="^(qwen|qwen_custom_voice|luxtts|chatterbox|chatterbox_turbo|tada|kokoro|openai_tts|elevenlabs_tts)$",
    )
...
class SpeakRequest(BaseModel):
    """Body for POST /speak — non-MCP REST surface that mirrors movabel.speak."""
...
class BatchGenerationItem(BaseModel):
    ...
    text: str = Field(..., min_length=1, max_length=10000)
    profile: Optional[str] = Field(
        None, description="Voice profile name or id for this item"
    )
    engine: Optional[str] = Field(
        None,
        pattern="^(qwen|qwen_custom_voice|luxtts|chatterbox|chatterbox_turbo|tada|kokoro|openai_tts|elevenlabs_tts)$",
...
class BatchGenerateRequest(BaseModel):
    ...
    items: List[BatchGenerationItem] = Field(
        ...
        description="List of texts to generate, each with optional profile/engine/language"
    )
    engine: Optional[str] = Field(
        None,
        pattern="^(qwen|qwen_custom_voice|luxtts|chatterbox|chatterbox_turbo|tada|kokoro|openai_tts|elevenlabs_tts)$",
        description="Default engine for items that don't specify one"
    )
    language: Optional[str] = Field(
        None,
        pattern="^(zh|en|ja|ko|de|fr|ru|pt|es|it|he|ar|da|el|fi|hi|ms|nl|no|pl|sv|sw|tr)$",
        description="Default language for items that don't specify one"
    )


class BatchGenerationItemStatus(BaseModel):
    """Status of a single item in a batch generation."""
...
class BatchGenerationResponse(BaseModel):
    """Response model for batch generation submission."""
...
class BatchGenerationStatusResponse(BaseModel):
    """Response model for batch generation status polling."""
    ...
    completed: int
    failed: int
    items: List[BatchGenerationItemStatus]


# ── Cloud TTS configuration models ────────────────────────────────────


class CloudTTSSettingsResponse(BaseModel):
    """Server-persisted settings for cloud TTS backends."""
...
class CloudTTSSettingsUpdate(BaseModel):
    """Partial update for cloud TTS settings."""

    openai_api_key: Optional[str] = Field(None, min_length=1, max_length=256)
    openai_base_url: Optional[str] = Field(None, max_length=512)
    openai_default_model: Optional[str] = Field(None, max_length=128)
    openai_default_voice: Optional[str] = Field(None, max_length=128)
    elevenlabs_api_key: Optional[str] = Field(None, min_length=1, max_length=256)
    elevenlabs_default_model: Optional[str] = Field(None, max_length=128)
...
class ReorderRequest(BaseModel):
    """Generic request payload for reordering items by ID list."""

    scene_ids: list[str] = Field(..., min_length=1, description="Ordered list of scene IDs")
