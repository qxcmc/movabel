...
def find_binary() -> Optional[Path]:
    ...
    candidates = [
        DIST_DIR / "movabel-server-cuda" / f"movabel-server-cuda{exe}",
        DIST_DIR / f"movabel-server{exe}",
    ]
    for c in candidates:
...
def write_reports(
    ...
    lines = [
        f"# Movabel E2E — {plat} — {started_at.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        f"Binary: `{binary}`  ",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Movabel E2E model generation test")
    p.add_argument("--binary", type=Path, help="Path to movabel-server binary (overrides auto-detect)")
...
def main() -> int:
    ...
    # Tempdir + log path
    data_dir = Path(tempfile.mkdtemp(prefix="movabel-e2e-"))
