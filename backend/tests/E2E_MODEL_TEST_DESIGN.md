...
|----------|------|------------|
| macOS    | `backend/dist/movabel-server-cuda/movabel-server-cuda` | onedir (CUDA, rarely on Mac) |
| macOS    | `backend/dist/movabel-server`                          | onefile (CPU) |
| Windows  | `backend\dist\movabel-server-cuda\movabel-server-cuda.exe` | onedir (CUDA) |
| Windows  | `backend\dist\movabel-server.exe`                      | onefile (CPU) |
...
- **Data dir**: `tempfile.mkdtemp(prefix="movabel-e2e-")`. Deleted after the run unless `--keep-data-dir`. Profiles and generated WAVs land here.
...
```json
{
  ...
  "binary": "/abs/path/movabel-server",
  "binary_size_mb": 612,
...
```
# Movabel E2E — darwin-arm64 — 2026-04-16 12:34
...
- Don't touch the user's HF cache by default — let the server use `HF_HUB_CACHE` / `MOVABEL_MODELS_DIR`. Passing `--isolated-cache` would point both env vars at the tempdir for a true cold-start run (opt-in only; would re-download every time).
