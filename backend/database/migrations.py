"""Column-level migrations for the movabel SQLite database.

Why not Alembic?  movabel is a single-user desktop app shipping as a
...
def _migrate_mcp_bindings(engine, inspector, tables: set[str]) -> None:
    ...
    to a boolean: when true, ``movabel.speak`` rewrites input through the
