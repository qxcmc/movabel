"""ORM model definitions for the movabel SQLite database."""
...
class CaptureSettings(Base):
    ...
    # "Movabel would like to receive keystrokes from any application" dialog
...
class BatchJob(Base):
    """A batch generation job tracking multiple generations.

    generation_ids is stored as a comma-separated string of Generation.id
    values so we can join back to the generations table for status polling.
    """
    ...
    generation_ids = Column(Text, nullable=True)  # comma-separated
...
class MCPClientBinding(Base):
    ...
    itself via the ``X-Movabel-Client-Id`` HTTP header; direct-HTTP
    clients set it in their MCP config's ``headers`` block, the stdio
    shim forwards it from the ``MOVABEL_CLIENT_ID`` env var.
    """
    ...
    # When true, movabel.speak routes through the profile's personality LLM
