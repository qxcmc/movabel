...
Docker support makes Movabel easier to deploy, especially for:
...
```bash
# CPU-only version
docker run -p 8000:8000 -v movabel-data:/app/data \
  ghcr.io/liumubai/movabel:latest

# NVIDIA GPU version
docker run --gpus all -p 8000:8000 -v movabel-data:/app/data \
  ghcr.io/liumubai/movabel:latest-cuda

# AMD GPU version (experimental)
docker run --device=/dev/kfd --device=/dev/dri -p 8000:8000 \
  -v movabel-data:/app/data \
  ghcr.io/liumubai/movabel:latest-rocm
```
...
services:
  movabel:
    image: ghcr.io/liumubai/movabel:latest-cuda
    ports:
      - "8000:8000"
    volumes:
      - movabel-data:/app/data
...
Build and run:
```bash
docker build -t movabel .
docker run -p 8000:8000 -v $(pwd)/data:/app/data movabel
```
...
Build:
```bash
docker build -f Dockerfile.optimized -t movabel:slim .
```
...
docker run --device=/dev/kfd --device=/dev/dri \
  ...
  -p 8000:8000 -v movabel-data:/app/data \
  movabel:rocm
```
...
```bash
docker run -v movabel-data:/app/data \           # Profiles, generations, history
...
Configure Movabel via environment variables:
...
doctl compute droplet create movabel \
...
sh get-docker.sh
docker run --gpus all -d -p 80:8000 movabel:cuda
```
...
```bash
# Build and push
docker build -t gcr.io/your-project/movabel .
docker push gcr.io/your-project/movabel

# Deploy to Cloud Run
gcloud run deploy movabel \
  --image gcr.io/your-project/movabel \
  --platform managed \
  --region us-central1 \
  --memory 4Gi \
  --cpu 2 \
...
[build]
  image = "ghcr.io/liumubai/movabel:latest"
...
```bash
# Fix permissions
docker run --user $(id -u):$(id -g) -v $(pwd)/data:/app/data movabel
```
...
```bash
docker run -e GPU_MEMORY_FRACTION=0.5 movabel
```

Or use CPU-only:
```bash
docker run -e DEVICE=cpu movabel
```
...
```bash
docker run -v huggingface-cache:/root/.cache/huggingface movabel
```

Or use host cache:
```bash
docker run -v ~/.cache/huggingface:/root/.cache/huggingface movabel
```
...
Change host port:
```bash
docker run -p 8080:8000 movabel  # Use port 8080 instead
```
...
docker run -e GPU_MEMORY_FRACTION=0.8 movabel

# Allow GPU memory growth (prevents OOM)
docker run -e TF_FORCE_GPU_ALLOW_GROWTH=true movabel
```
...
View container logs:
```bash
docker logs -f movabel

# Or with compose
docker compose logs -f movabel
```
...
4. Report issues: [GitHub Issues](https://github.com/liumubai/movabel/issues)
