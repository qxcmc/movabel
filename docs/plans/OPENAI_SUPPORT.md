...
**Issue:** [#10 OpenAI API compatibility](https://github.com/liumubai/movabel/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from Movabel, allowing any tool, library, or application that speaks the OpenAI Audio API to use Movabel as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph movabel [Movabel Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with Movabel
- **LLM frameworks**: LangChain, AutoGen, etc. can use Movabel for TTS
...
- `voice` parameter maps to Movabel profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a Movabel profile.
