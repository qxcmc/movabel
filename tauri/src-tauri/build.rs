...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    {
        // movabel.icon is in tauri/assets/movabel.icon (one level up from src-tauri)
        let icon_source = format!("{}/../assets/movabel.icon", project_root);
        ...
        if std::path::Path::new(&icon_source).exists() {
            ...
            // Generate movabel.icns from the source PNG via sips + iconutil
            let icns_path = format!("{}/movabel.icns", gen_dir);
            if !std::path::Path::new(&icns_path).exists() {
                let source_png = format!("{}/Assets/Movabel.png", icon_source);
                if std::path::Path::new(&source_png).exists() {
                    let iconset_dir = format!("{}/movabel.iconset", gen_dir);
                    ...
                    match iconutil_output {
                        Ok(out) if out.status.success() => {
                            println!("Generated movabel.icns");
                        }
                        Ok(out) => {
    ...
    {
        let required = ["Assets.car", "movabel.icns", "partial.plist"];
